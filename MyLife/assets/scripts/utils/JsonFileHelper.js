...
function loadJsonFromFile(file , callback){
    ...
    cc.loader.load(fileUri , function(err , res){
        var b = new Base64();
        var str = b.decode(JSON.stringify(res));
        cc.info('load['+ fileUri +'], err['+err+'] result: ' + JSON.stringify(res));
        callback(str);
    });
}

//
function writeJsonToFile(file , content){
    var fileUri = cc.url.raw(file);
    cc.info("writeToFile , isNative "+cc.sys.isNative);
    if (cc.sys.isNative) {
        jsb.fileUtils.writeToFile(new String(content) , file);
        cc.info("is native");
    }
